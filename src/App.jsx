    ...
    <div className="min-h-screen bg-[#faf9f6] font-sans text-gray-900 pb-20">
        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="bg-black text-white px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-[#d4af37] transition-all flex items-center gap-2"
            >
              <PlusCircle size={16} />
      ...
      <div className="bg-white border-b sticky top-20 z-30">
        <div className="container mx-auto px-4 py-3 flex flex-wrap items-center justify-center gap-4">
          <div className="flex bg-gray-100 p-1 rounded-full">
            {GENDERS.map(g => (
              <button key={g} onClick={() => setSelectedGender(g)} className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all ${selectedGender === g ? 'bg-black text-white' : 'text-gray-500'}`}>
                {g}
              </button>
            ))}
          </div>
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {["Todo", ...AGE_GROUPS].map(age => (
              <button key={age} onClick={() => setSelectedAge(age)} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase whitespace-nowrap border ${selectedAge === age ? 'bg-[#d4af37] text-white border-[#d4af37]' : 'bg-white text-gray-400 border-gray-200'}`}>
                {age}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4">
          {CATEGORIES.map(cat => (
            <div key={cat.id} onClick={() => setSelectedCategory(cat.id === selectedCategory ? null : cat.id)} className={`group flex-shrink-0 cursor-pointer flex flex-col items-center transition-all ${selectedCategory === cat.id ? 'scale-105 opacity-100' : 'opacity-70'}`}>
              <div className={`w-20 h-20 md:w-28 md:h-28 rounded-full overflow-hidden border-2 ${selectedCategory === cat.id ? 'border-[#d4af37]' : 'border-transparent'}`}>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-6 opacity-60 grayscale hover:grayscale-0 transition-all">
            {BRANDS.map((brand, idx) => (
              <div key={idx} className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-white rounded-lg p-1.5 shadow-sm border border-gray-100 hover:border-[#d4af37] cursor-pointer" title={brand.name}>
                <img src={brand.logo} alt={brand.name} className="max-w-full max-h-full object-contain" onError={(e) => {e.target.src = `https://ui-avatars.com/api/?name=${brand.name}&size=32`;}} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 mb-10 flex flex-wrap justify-center gap-3">
        {["Todas", ...CONDITIONS].map(cond => (
          <button key={cond} onClick={() => setSelectedCondition(cond)} className={`px-5 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest border-2 transition-all ${selectedCondition === cond ? 'bg-black border-black text-white' : 'bg-white border-gray-100 text-gray-400'}`}>
            {cond}
          </button>
        ))}
      </div>
      ...
      <main className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {filteredProducts.map(product => (
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 group hover:shadow-xl transition-all flex flex-col">
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-4 pt-3 border-t flex items-center justify-between text-gray-300">
                  <Heart size={16} className="hover:text-red-500 cursor-pointer transition-colors" />
                  <button className="bg-gray-50 text-black border border-gray-100 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all">
                    Ver más
                  </button>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl p-8 overflow-y-auto max-h-[90vh]">
            <h2 className="text-2xl font-black tracking-tighter uppercase italic mb-6 text-center">+Roma Vender</h2>
            <form onSubmit={handleUpload} className="space-y-6">
              <input type="text" placeholder="Título" className="w-full border-b py-3 text-sm outline-none" onChange={(e) => setNewProduct({...newProduct, title: e.target.value})} required />
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="Precio $" className="w-full border-b py-3 text-sm outline-none" onChange={(e) => setNewProduct({...newProduct, price: e.target.value})} required />
                <input type="text" placeholder="Talle" className="w-full border-b py-3 text-sm outline-none" onChange={(e) => setNewProduct({...newProduct, size: e.target.value})} />
              </div>
              <button type="submit" className="w-full bg-black text-white py-4 rounded-[2rem] font-black uppercase text-xs hover:bg-[#d4af37]">Publicar ahora</button>
            </form>
          </div>
        </div>
      )}
