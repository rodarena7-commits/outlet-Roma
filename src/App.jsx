...
import { 
  ...
  ChevronLeft, ChevronRight as ChevronRightIcon, Info, Download, MessageCircle,
  Send, DollarSign, Crop, Move, Maximize2, Minimize2, Ban
} from 'lucide-react';
...
const CATEGORIES = [
  ...
  { id: 'electrodomesticos', name: 'Electrodomésticos', image: 'https://images.unsplash.com/photo-1584269600464-37b1b58a9fe7?auto=format&fit=crop&q=80&w=200' },
];
...
export default function App() {
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar esta publicación?")) {
      ...
      // También eliminar las preguntas asociadas
  ...
  const handlePayment = () => {
    // Construir el total para MercadoPago
    ...
    const productosTexto = cart.map(item => 
      `- ${item.title} (Talle: ${item.category === 'calzado' ? item.shoeSize : item.size}) - $${item.price}`
    ).join('\n');
    ...
    cart.forEach(async (item) => {
      await handleMarkAsSold(item.id);
    });
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  {/* Medidas */}
                  {(selectedProduct.shoulderToShoulder || selectedProduct.armpitToArmpit || selectedProduct.length || 
                    selectedProduct.waist || selectedProduct.hip || selectedProduct.inseam || selectedProduct.outseam) && (
                    <div className="mb-6">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <Clock size={18} /> 
                {selectedTab === 'pendientes' ? 'Publicaciones Pendientes' : 
                 selectedTab === 'publicadas' ? 'Publicaciones Publicadas' : 'Publicaciones Vendidas'}
              </h3>
              <div className="space-y-6">
                {(selectedTab === 'pendientes' ? pendingProducts : 
                  selectedTab === 'publicadas' ? products : 
                  [...products, ...pendingProducts].filter(p => p.sold)).map(product => (
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="p-8 md:p-12">
              ...
              <form onSubmit={handleUpdateProduct} className="space-y-6">
                ...
                {/* Medidas para parte superior */}
                {(editingProduct.category === 'remeras' || editingProduct.category === 'camisas' || 
                  editingProduct.category === 'buzos' || editingProduct.category === 'vestidos' || 
                  editingProduct.category === 'camperas') && (
                  <div className="space-y-4">
                ...
                {/* Medidas para parte inferior */}
                {(editingProduct.category === 'pantalones' || editingProduct.category === 'jeans' || 
                  editingProduct.category === 'short') && (
                  <div className="space-y-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="flex-1 p-8 md:p-12 overflow-y-auto no-scrollbar">
              ...
              <form onSubmit={handleUpload} className="space-y-6">
                ...
                {/* Medidas para parte superior */}
                {(newProduct.category === 'remeras' || newProduct.category === 'camisas' || 
                  newProduct.category === 'buzos' || newProduct.category === 'vestidos' || 
                  newProduct.category === 'camperas') && (
                  <div className="space-y-4">
                    <h4 className="font-bold text-sm">Medidas parte superior (cm)</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <input 
                ...
                {/* Medidas para parte inferior */}
                {(newProduct.category === 'pantalones' || newProduct.category === 'jeans' || 
                  newProduct.category === 'short') && (
                  <div className="space-y-4">
                    <h4 className="font-bold text-sm">Medidas parte inferior (cm)</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <input 
