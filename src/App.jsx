...
export default function App() {
  ...
  const handleStartChat = async (targetUser) => {
    console.log("🚀 handleStartChat llamado con:", targetUser);
    ...
    if (targetUser.uid === user.uid) {
      console.log("❌ Error: intentando chatear consigo mismo");
      alert("No puedes chatear contigo mismo");
      return;
    }

    console.log("✅ Validaciones pasadas, buscando chat existente...");
    
    // Buscar si ya existe un chat entre estos usuarios
    const existingChat = chats.find(chat => 
      chat.participants.includes(user.uid) && 
      chat.participants.includes(targetUser.uid) &&
      (chat.type === 'direct_message' || !chat.type)
    );
    ...
    if (existingChat) {
      ...
      try {
        const chatRef = await addDoc(collection(db, "chats"), chatData);
        console.log("✅ Chat creado con ID:", chatRef.id);
        
        // Enviar mensaje de bienvenida
        await addDoc(collection(db, "messages", chatRef.id, "chatMessages"), {
          senderId: 'system',
          senderName: 'Sistema',
          text: `👋 Chat iniciado entre ${user.name} y ${targetUser.name}`,
          timestamp: Timestamp.now(),
          read: false,
          system: true
        });
        ...
        await addDoc(collection(db, "notifications"), {
          ...
          type: "new_chat",
          title: "Nuevo chat",
          message: `${user.name} ha iniciado un chat contigo.`,
          chatId: chatRef.id,
          read: false,
          createdAt: Timestamp.now()
        });
        ...
        alert("Error al crear el chat. Revisa la consola.");
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] font-sans text-slate-900 pb-20 selection:bg-[#d4af37]/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    {/* Botón para iniciar chat con cualquier usuario (no admin) */}
                    {user && user.uid !== selectedProduct.userId && !user.isAdmin && (
                      <button
                        onClick={() => {
                          const targetUser = users[selectedProduct.userId];
      ...
      {/* --- Modal de Perfil de Usuario (con pestaña Chat y botón Iniciar Chat) --- */}
      {selectedUserProfile && (
                ...
                <div className="flex gap-3 mt-4">
                  {user && user.uid !== selectedUserProfile && (
                    <>
                      ...
                      {/* Botón para iniciar chat con este usuario */}
                      <button
                        onClick={() => {
                          console.log("👤 Botón 'Iniciar chat' clickeado para usuario:", users[selectedUserProfile]);
                          const targetUser = users[selectedUserProfile];
                          handleStartChat(targetUser);
                          setSelectedUserProfile(null);
                        }}
                        className="bg-green-600 text-white px-6 py-2 rounded-full text-xs font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-green-700 transition-all"
                      >
      ...
      {showAdminModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            {/* --- Sección de Chats con Usuarios (Admin) - VERSIÓN SIMPLIFICADA --- */}
            <div className="mt-12">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div className="relative bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 h-18 flex items-center justify-around z-50 px-6 shadow-2xl">
        ...
        {/* CHAT - NUEVO BOTÓN (reemplaza al corazón) */}
        <button 
          className="text-slate-400 p-2 relative" 
          onClick={() => {
            if (user) {
              setSelectedUserProfile(user.uid);
              // Forzar la pestaña de chats después de que se abra el perfil
              setTimeout(() => {
