...
export default function App() {
  ...
  // Cargar configuración de descuentos desde Firestore
  useEffect(() => {
    const loadDiscountConfig = async () => {
      ...
      snapshot.docs.forEach(doc => {
        if (doc.id === "saleConfig") {
          globalConfig = doc.data();
        } else if (doc.id === "categoryDiscounts") {
          categoryConfig = doc.data().discounts || {};
        }
      });
      if (Object.keys(globalConfig).length > 0) {
        setSaleConfig(prev => ({ ...prev, ...globalConfig }));
      }
      setCategoryDiscounts(categoryConfig);
    };
    loadDiscountConfig();
  }, []);

  // Efecto para guardar automáticamente la configuración cuando cambie
  useEffect(() => {
    const saveConfig = async () => {
      await setDoc(doc(db, "config", "saleConfig"), {
        promo: saleConfig.promo,
        active: saleConfig.active,
  ...
  useEffect(() => {
    const saveCategoryDiscounts = async () => {
      await setDoc(doc(db, "config", "categoryDiscounts"), {
  ...
  // Función para marcar como vendido (toggle)
...
const handleMarkAsSold = async (productId, buyerId, paymentMethod = 'mercadopago', receiptImageUrl = null) => {
  ...
  // Función para calcular precio con descuento (prioridad: categoría > global)
  const getDiscountedPrice = (product) => {
    if (!saleConfig.active) return product.price;

    // Verificar fecha de expiración
    if (saleConfig.fechaFin) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const expireDate = new Date(saleConfig.fechaFin);
      expireDate.setHours(0, 0, 0, 0);
      if (today > expireDate) return product.price;
    }

    let discountPercent = 0;
    // Descuento por categoría
    if (categoryDiscounts[product.category]) {
      discountPercent = categoryDiscounts[product.category];
    } else if (saleConfig.promo) {
      // Descuento global
      discountPercent = parseFloat(saleConfig.promo) || 0;
    }

    if (discountPercent <= 0 || discountPercent >= 100) return product.price;
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              {chat.unreadCount?.[user.uid] > 0 && (
                                <span className="mt-1 inline-block bg-green-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full">
                                  {chat.unreadCount[user.uid]} nuevos
                                </span>
                              )}
                            </div>
                          </div>
      ...
      <footer className="bg-white border-t border-slate-100 py-6 mt-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-8">
            ...
            {/* TikTok */}
            <a
              href="https://www.tiktok.com/@show.roma.importados?lang=en&is_from_webapp=1&sender_device=mobile&sender_web_id=7629826085513758215"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:text-black transition-colors p-2"
              title="TikTok"
            >
            ...
            {/* WhatsApp (PNG) */}
            <a
              href="https://chat.whatsapp.com/DmTaiFnOOfUKIF027qSs50?mode=gi_t"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:opacity-80 transition-opacity p-2"
              title="WhatsApp"
            >
            ...
            {/* MercadoLibre (PNG) */}
            <a
              href="https://www.mercadolibre.com.ar/pagina/outletmasroma#from=share_eshop"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:opacity-80 transition-opacity p-2"
              title="MercadoLibre"
            >
              <img src="/mercadolibre.png" alt="MercadoLibre" className="w-6 h-6 object-contain" />
            </a>

            {/* Google Maps */}
            <a
              href="https://share.google/E2rhiBmqszTWZ8G3x"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:text-[#EA4335] transition-colors p-2"
              title="Google Maps"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* Instagram Outlet +Roma */}
            <a
              href="https://www.instagram.com/outlet.masroma?igsh=Z3E3dXFxNHoxcm4z"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:text-[#E4405F] transition-colors p-2"
              title="Instagram Outlet"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* Instagram Showroom +Roma */}
            <a
              href="https://www.instagram.com/showroom.masroma?igsh=aHFydXlseG4weThu"
              target="_blank"
              rel="noopener noreferrer"
              className="text-slate-500 hover:text-[#E4405F] transition-colors p-2"
              title="Instagram Showroom"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>
          ...
          <p className="text-center text-[10px] text-slate-400 mt-4">
            © {new Date().getFullYear()} +Roma Showroom. Todos los derechos reservados.
          </p>
        </div>
      </footer>
                  ...
                  {(selectedProduct.shoulderToShoulder || selectedProduct.armpitToArmpit || selectedProduct.length || selectedProduct.waist || selectedProduct.hip || selectedProduct.inseam || selectedProduct.outseam) && (
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        {selectedProduct.receipt && (
                    <div className="mb-4">
                      <h4 className="font-bold text-sm mb-2">Comprobante de pago</h4>
                      <img 
                        src={selectedProduct.receipt} 
                        alt="Comprobante" 
                        className="max-w-full max-h-48 rounded-lg cursor-pointer border"
                        onClick={() => window.open(selectedProduct.receipt, '_blank')}
                      />
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-12 p-6 bg-slate-50 rounded-3xl">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Tag size={18} /> Configurar Banner y Descuentos</h3>
              <div className="space-y-6">
                {/* Título del Banner */}
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="number"
                      className="flex-1 bg-white border p-4 rounded-2xl outline-none focus:border-black"
                ...
                {/* Vigencia (texto libre) */}
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase text-slate-400">
                    Vigencia (texto visible en banner)
                  </label>
                  <input
                    className="w-full bg-white border p-4 rounded-2xl outline-none focus:border-black"
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {CATEGORIES.map(cat => (
                      <div key={cat.id} className="flex items-center gap-2 bg-white p-2 rounded-xl border">
                        <span className="text-xs font-medium capitalize">{cat.name}</span>
                        <input
                          type="number"
                          placeholder="%"
                          className="w-16 text-center bg-slate-50 rounded-lg p-1 text-sm"
                ...
                <div key={item.id} className="flex gap-4 group">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      {(() => {
                        const discounted = getDiscountedPrice(item);
                        const hasDiscount = saleConfig.active && discounted !== item.price;
                        return (
                          <div className="flex flex-col items-end">
                            <span className={`text-sm ${hasDiscount ? 'text-slate-400 line-through' : 'font-black'}`}>
