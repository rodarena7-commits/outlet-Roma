...
const CATEGORIES = [
  ...
  { id: 'electrodomesticos', name: 'Electrodomésticos', image: '/electro.png' }, // Cambio: Imagen local electro.png
];
...
export default function App() {
  ...
  const compressImage = (dataUrl, maxWidth = 1024, maxHeight = 1024, quality = 0.8) => {
  ...
  const uploadImageToStorage = async (imageDataUrl, path) => {
    try {
  ...
  const handleEditProductImagesUpload = async (e) => {
    const files = Array.from(e.target.files);
    const newImages = [];
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      alert("Subiendo imágenes... esto puede tomar unos segundos");

      // Subir imágenes a Storage en lugar de guardarlas en Firestore
      const imageUrls = [];
      for (let i = 0; i < newProduct.images.length; i++) {
        const imageDataUrl = newProduct.images[i];
        const imagePath = `products/${user.uid}/${Date.now()}_${i}.jpg`;
        const downloadUrl = await uploadImageToStorage(imageDataUrl, imagePath);
      ...
      const productToAdd = {
        ...
        images: imageUrls, // Ahora son URLs de Storage, NO base64
      ...
      alert("Error al publicar. Por favor, intenta con imágenes más pequeñas.");
    }
  };
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      // Las imágenes que son base64 (nuevas) deben subirse a Storage
      const newImageUrls = [];
      for (let i = 0; i < imageUrls.length; i++) {
        const img = imageUrls[i];
        // Si la imagen es base64 (empieza con data:), subirla a Storage
        if (img.startsWith('data:')) {
          const imagePath = `products/${user.uid}/${Date.now()}_${i}.jpg`;
          const downloadUrl = await uploadImageToStorage(img, imagePath);
      ...
      alert("Error al actualizar. Por favor, intenta con imágenes más pequeñas.");
    }
  };
  ...
  // --- Modificación: Flujo de pago corregido ---
  const handlePayment = async () => {
    const total = cartTotal;
    
    // Abrir MercadoPago en una nueva pestaña sin relación de navegación
    const mpWindow = window.open(
      "https://link.mercadopago.com.ar/masroma?amount=" + total, 
      "_blank",
      "noopener,noreferrer"
    );
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            {/* --- Botón para contactar vendedor (Admin) --- */}
